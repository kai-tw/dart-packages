...
/// redactor can enforce.
///
/// ## Why the code is an `int`
///
/// A `String` code reads better (`'connection-refused'`), and is exactly the
/// field that turns into a PII channel the first time someone writes
/// `'failed for $bookId'`. An `int` cannot carry a name, a path or a sentence
/// at all, whatever a future implementer does — and it is what the values
/// actually are: an OS errno, an HTTP status, a platform result code.
///
/// The redactor additionally band-clamps it, so an implementer who returns a
/// timestamp or a record count degrades to type-name-only rather than
